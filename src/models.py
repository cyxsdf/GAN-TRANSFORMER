...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dim, seq_len, hidden_dim=256, num_layers=2, num_heads=4, kernel_sizes=[3, 5, 7]):
        ...
        for i, kernel_size in enumerate(kernel_sizes):
            ...
            self.conv_layers.append(
                nn.Sequential(
                    ...
                    nn.LeakyReLU(0.2),
                    nn.MaxPool1d(kernel_size=2, stride=2)  # 减半序列长度
                )
            )

        # 计算CNN输出后的序列长度和特征维度
        self.cnn_output_len = seq_len // 2  # 因为使用了MaxPool1d(2,2)
        self.cnn_output_dim = hidden_dim  # 多个卷积核输出拼接后的总维度

        # Transformer编码器层
        transformer_layer = nn.TransformerEncoderLayer(
            d_model=self.cnn_output_dim,
            nhead=num_heads,
            dim_feedforward=hidden_dim * 4,  # 通常为d_model的4倍
            dropout=0.1,
            activation='gelu',
            batch_first=True  # 以batch为第一维度
        )
        ...
        # 位置编码（可选，Transformer本身没有位置信息）
        self.pos_encoder = nn.Embedding(self.cnn_output_len, self.cnn_output_dim)

    def forward(self, x):
        # x shape: [batch_size, seq_len, input_dim]

        # 调整维度以适应CNN: [batch_size, input_dim, seq_len]
        x = x.transpose(1, 2)

        # 多尺度CNN特征提取
        cnn_outputs = []
        for conv in self.conv_layers:
            cnn_out = conv(x)  # [batch_size, out_channels, cnn_output_len]
            cnn_outputs.append(cnn_out)

        # 拼接多尺度特征
        cnn_combined = torch.cat(cnn_outputs, dim=1)  # [batch_size, hidden_dim, cnn_output_len]
        ...
        # 调整维度以适应Transformer: [batch_size, cnn_output_len, hidden_dim]
        ...
        # Transformer处理
        transformer_out = self.transformer_encoder(transformer_input)  # [batch_size, cnn_output_len, hidden_dim]

        # 取最后一个时间步的输出（也可以用全局平均池化）
        last_out = transformer_out[:, -1, :]  # [batch_size, hidden_dim]

        # 分类
        pred = self.classifier(last_out)  # [batch_size, 1]
...
class TRANSLATEModel(nn.Module):
    ...
    def forward(self, src, tgt, phase='train', eval_start=False):
        ...
        if phase != 'test':
            ...
            if eval_start:
                if self.missing == 'L':
                    x_l = multi  # 使用[Uni]或[Bi]标记作为生成起始点
                elif self.missing == 'A':
                    x_a = multi
                elif self.missing == 'V':
                    x_v = multi
